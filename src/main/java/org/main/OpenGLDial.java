...
public class OpenGLDial extends SecondaryGraph implements GLEventListener {
    ...
    public OpenGLDial() {
        this.maxValue = 1;
        this.minValue = -1;
        verticies = Buffers.newDirectFloatBuffer(sampleCount * 4 * Buffers.SIZEOF_FLOAT);
    }
    ...
    public void display(GLAutoDrawable glAutoDrawable) {
        ...
        try {
            ...
            OpenGL.drawBox(gl, backgroundColor, leftPlotX, bottomPlotY, rightPlotX - leftPlotX, topPlotY - bottomPlotY);
            ...
            float[] datasetColor = new float[]{dataset.getColor().getRed() / 255.0f, dataset.getColor().getGreen() / 255.0f, dataset.getColor().getBlue() / 255.0f, 1f};

            OpenGL.buffer.rewind();
            setVertices(glAutoDrawable);
            OpenGL.drawTrianglesXY(gl, GL3.GL_TRIANGLE_STRIP, datasetColor, OpenGL.buffer, (int)(sampleCount * percent));
            OpenGL.buffer.rewind();
        } catch (Exception e) {
    ...
    private void setVertices(GLAutoDrawable glAutoDrawable) {
        ...
        for (double angle = 0; angle < Math.PI; angle += increment) {
            OpenGL.buffer.put(-1 * (originX + ((float) Math.cos(angle) * radiusInner[0])));
            OpenGL.buffer.put(originY + ((float)Math.sin(angle) * radiusInner[1]));
            OpenGL.buffer.put(-1 * (originX + ((float) Math.cos(angle) * radiusOuter[0])));
            OpenGL.buffer.put(originY + ((float)Math.sin(angle) * radiusOuter[1]));
        }
    }
